...
class Learndash_Wpforo_Public {
	...
	/**
	 * Board form course access
	 *
	 * @param  mixed $template
	 * @return void
	 */
	public function learndash_wpforo_board_wpftpl( $template ) {
		global $wpdb;
		if ( strpos( $template, 'topic.php' ) ) {
			$board_obj = WPF()->board->get_current();
			$board_id = $board_obj['boardid'];
			$board_forum = WPF()->current_object['forum'];
			if ( isset( $board_forum['forumid'] ) && $board_forum['forumid'] != '' ) {
				$b_forumid = $board_forum['forumid'];
			} else {
				return $template;
			}
			$ld_b_forum_info = get_option( 'ld_board_forum_' . $board_id. '_' . $b_forumid );
			$associated_courses = ( isset( $ld_b_forum_info['ld_course_selector_dd'] ) ) ? $ld_b_forum_info['ld_course_selector_dd'] : '';
			$allow_post = ( isset( $ld_b_forum_info['ld_post_limit_access'] ) ) ? $ld_b_forum_info['ld_post_limit_access'] : '';
			$allow_forum_view = ( isset( $ld_b_forum_info['ld_allow_forum_view'] ) ) ? $ld_b_forum_info['ld_allow_forum_view'] : '';
			$message_without_access = ( isset( $ld_b_forum_info['ld_message_without_access'] ) ) ? $ld_b_forum_info['ld_message_without_access'] : '';
	...
	public function learndash_wpforo_footer_hook_for_board() {
		if ( in_array( WPF()->current_object['template'], array( 'post', 'topic' ) ) ) {
			$board_obj = WPF()->board->get_current();
			$board_id = $board_obj['boardid'];
			$board_forum = WPF()->current_object['forum'];
